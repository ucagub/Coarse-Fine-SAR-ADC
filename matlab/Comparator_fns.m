classdef Comparator_fns
    ...
    methods
        ...
        function [c,ceq] = power_constraint(obj, u)
            %comparator power constraint
            %inputs: x = [fine_comp_input_referred_noise coarse_comp_input_referred_noise]
            Vref = 1;
            N = obj.N;
            k = obj.k;
            Plsb = obj.Plsb;
...
%             c = [];
            c(1) = u(1) - u(2);
%             c(1) = u(2) - u(1);
            ceq = ((N-k)/u(1)^2 + k/u(2)^2)*(lsb^2)*Plsb - Pbudget;
        end
        function y = MSE_prox(obj, u)
            %MSE formula from the paper
            y = MSE(u, obj.k, obj.N);
        end
        function [c,ceq] = power_constraint_prox(obj, u)
            %inputs: x = [fine_comp_input_referred_noise coarse_comp_input_referred_noise]
            N = obj.N;
            k = obj.k;
            Vref = obj.Vref;
...
%             c(1)= x(1) - x(2);
        ...
        function [c, ceq] = mse_constraint_prox(obj,u)
            %inputs: x = [fine_comp_input_referred_noise coarse_comp_input_referred_noise]
%             c(1) = -x(1) + 0.1031 ;
%             c(2) = x(2) - 0.35;
%             c = [];
            c(1) = u(1) - u(2);
%             c(1) = u(2) - u(1);
            ceq = MSE(u, obj.k, obj.N) - obj.mse_budget;
            
        end
        
        function y = power_prox(obj, u)
            %inputs: u = [fine_comp_input_referred_noise coarse_comp_input_referred_noise]
            N = obj.N;
            k = obj.k;
            Vref = obj.Vref;;
            Plsb = obj.Plsb;
