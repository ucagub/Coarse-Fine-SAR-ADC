...
%     zlabel('ENOB')
    ...
    sf = fit([x', y'],z','poly23');
    label = ['cs-cs-3d-200ksps-loaded-with-noise/coarse_' coarse_dac_type 'fine_' fine_dac_type 'k_' num2str(k) '.mat'];
    save(label)
    label_var = ['cs-cs-3d-200ksps-loaded-with-noise/coarse_' coarse_dac_type 'fine_' fine_dac_type 'k_' num2str(k) 'sf' '.mat'];
    save(label_var, 'sf');
    close;
    figure;
    plot(sf,[x',y'],z');
